...
$dari_no=addslashes($_POST['nourut']);
...
$idbaru=$rows1['id'];
$sql3="SELECT * FROM asset WHERE id='$idcdesa' && no_persil='$no_persil' && kelas_desa='$kelas_desa' && klasifikasi='$klasifikasi'";
$result=mysql_query($sql3);
$rows3=mysql_fetch_array($result);
$idsejarah=$rows3['id'];
$k_idsejarah=$rows3['k_id'];
if ($luas_sisaupdate) {
...
$insert=mysql_query("insert into assetbaru(name, id, k_id, no_persil, kelas_desa, luas_tersedia, luas_terpindah, luas_sisa, ipeda, diperoleh, tahun, no_kohir, atasnama, dipindah, tahun_pindah, no_kohir_pindah, atasnama_pindah, status, klasifikasi, nocdesa, idcdesa) values('$name','$idbaru','$Max_id','$no_persil','$kelas_desa','$luas_tersedia','$luas_terpindah','$luas_sisa','$ipeda','$diperolehsejarah','$tahunsejarah','$dari_no','$dari_nama','$dipindah','$tahun_pindah','$no_kohir_pindah','$atasnama_pindah','$status','$klasifikasi','$nocdesa','$idcdesa')");

$insert3=mysql_query("insert into sejarahbaru(id, k_id, no_persil, kelas_desa, luas_terpindah, metode, tahun, dari_no, dari_nama, ke_no, ke_nama, klasifikasi) values('$idsejarah','$k_idsejarah','$no_persil','$kelas_desa','$luas_tersedia','$diperolehsejarah','$tahunsejarah','$dari_no','$dari_nama','$Max_no','$namapindah','$klasifikasi')");

$insert4=mysql_query("insert into ketbaru(id, k_id, no_persil, kelas_desa, luas_terpindah, ipeda, dipindah, tahun_pindah, no_kohir_pindah, atasnama_pindah, klasifikasi, cdesaterkait) values('$id','$k_id','$no_persil','$kelas_desa','$luas_tersedia','$ipeda','$diperolehsejarah','$tahunsejarah','$Max_no','$namapindah','$klasifikasi','$nocdesa')");

$update=mysql_query("Update assetbaru set luas_tersedia='$luas_sisaupdate', luas_terpindah='$luas_tersedia', luas_sisa='$luas_sisaupdate', status='$statusupdate' WHERE k_id='$_GET[k_id]' AND id='$_GET[id]'");

if($insert2 & $insert & $insert3 & $insert4 & $update){
  $insertGoTo = "lihatDetailDataPemilik.php?id=" . $row_Recordset1['id'] . "";
...
mysql_select_db($database_eTanah, $eTanah);
$query_Recordset1 = "SELECT * FROM assetbaru WHERE k_id='$_GET[k_id]' AND id='$_GET[id]' ORDER BY k_id ASC";
...
<input type="hidden" name="idcdesa" value="<?php echo $row_Recordset1['idcdesa']; ?>" size="32" />
<input type="hidden" name="MM_insert" value="form1" />
<input type="hidden" name="dipindah" value="" size="32" />
<input type="hidden" name="notampung" value="" size="32" />
<input type="hidden" name="notampung" value="" size="32" />
<input type="hidden" name="nourut" value="<?php echo $row_Recordset2['no']; ?>" size="32" />
...
<input type="hidden" name="nocdesa" value="<?php echo $row_Recordset1['nocdesa']; ?>" size="32" />
